...
data = pd.DataFrame({'Kts': normalize(kts[:700]), 'Dts': normalize(dts[:700]), 'Lts': normalize(lts[:700])}, columns=col_list)
...
cdc = result.get_causal_direction_counts(n_directions=8, min_causal_effect=0.05, split_by_causal_effect_sign=True)
