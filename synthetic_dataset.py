...
class SyntheticDataset:
    ...
    def generate_data(self):
        ...
        for t in range(10, self.time_steps):
            ...
            self.X2.append(C.get('c3') * self.X1[t - Tao.get('t2')] + ey[t])
            self.X3.append(C.get('c1') ** ((self.X1[t - Tao.get('t4')]) / 2 + ez[t]))
            self.X4.append(C.get('c2') * self.X1[t - Tao.get('t3')] + er[t])
            self.X5.append(C.get('c1') * self.X3[t - Tao.get('t1')] + self.X4[t - Tao.get('t2')] + ey[t])
...
if __name__ == '__main__':
    ...
    er = np.random.normal(0, 0.25, time_steps)
